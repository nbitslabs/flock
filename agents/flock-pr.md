...
You will receive:
1. **Task context**: The original task or goal that prompted these changes (e.g., "Issue #39: Add PR agent")
...
1. **Review recent commits**: Run `git log` to see what commits were made and understand the progression of changes
2. **Check for existing PR**: Use `gh pr list` to check if a PR already exists for this branch
3. **Analyze changes**: Look at the files changed and understand the scope of modifications
...
6. **Return results**: Provide the PR URL and a summary of what you did (new PR created or existing PR updated)
